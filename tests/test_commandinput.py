...
class TestCommands:
    ...
    def test_abstract_command_execute(self, fake_started_async_parser):
        command = commandinput.Command()
        with pytest.raises(NotImplementedError):
            command.execute_command(fake_started_async_parser)

    def test_Command_execute_state(self, fake_started_async_parser, monkeypatch):
        command = commandinput.Command()
        monkeypatch.setattr(command, "_apply", lambda x: "a")
        res = command.execute_command(fake_started_async_parser)
        assert res == "a"
        assert command._executed == True

    @pytest.mark.asyncio
    async def test_ListProvidersCommand(self,
                                        fake_started_async_parser,
                                        periodic_provider_factory):
        command1 = commandinput.ListProvidersCommand()

        res1 = command1.execute_command(fake_started_async_parser)
        ...
        fake_started_async_parser.add_content_provider(provider1)
        fake_started_async_parser.add_content_provider(provider2)

        res2 = command2.execute_command(fake_started_async_parser)
    ...
    async def test_ListHooksCommand(self,
        ...
        for i in range(3):
            ...
            fake_started_async_parser.add_content_provider(provider)

        hook1 = hook_factory.get_hook()
        fake_started_async_parser.get_providers()[1].add_hook(hook1)
        ...
        fake_started_async_parser.get_providers()[1].add_hook(hook2)

        command3 = commandinput.ListHooksCommand(1)
        res3 = command3.execute_command(fake_started_async_parser)
        assert res3 == [hook1, hook2]

    @pytest.mark.asyncio
    async def test_NewProviderCommand(self, fake_started_async_parser):
        ...
        assert fake_started_async_parser.get_providers() == []

        res = command.execute_command(fake_started_async_parser)
        await asyncio.sleep(0.01)

        assert command._provider == res
        assert fake_started_async_parser.get_providers() == [res]
        assert res._asyncio_running
        assert res.a == 0
        assert res.b == 1

    @pytest.mark.asyncio
    async def test_NewHookCommand(self, fake_started_async_parser):
        ...
        for i in range(3):
            provider_command = commandinput.NewProviderCommand("Provider1",
                                                               args=[0],
                                                               kwargs={"b": 1})
            provider_command.execute_command(fake_started_async_parser)
            await asyncio.sleep(0.01)
        ...
        providers = fake_started_async_parser.get_providers()
